...
defineEmits(['add-ammount', 'reduce-quantity', 'add-cart','remove-item' ,'clear-cart'])

const totalPagar = computed(() => {
  return props.cart.reduce((total, acum)=> total + (acum.precio * acum.cantidad),0)
})
    ...
    <div class="container-xl">
                ...
                <tbody>
                  <tr v-for="item in cart">
                    ...
                    <td class="flex align-items-start gap-4">
                      <button @click="$emit('reduce-quantity', item.id)" type="button" class="btn btn-dark">
                        -
                      </button>
                      {{ item.cantidad }}
                      <button @click="$emit('add-ammount', item.id)" type="button" class="btn btn-dark">
                        +
                      </button>
                    </td>
                    <td>
      ...
      <div class="row mt-5">
        <div class="col-md-6 text-center text-md-start pt-5">
          ...
          <button @click="$emit('add-cart', guitar)" type="button"
